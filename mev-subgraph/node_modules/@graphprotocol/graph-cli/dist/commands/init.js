...
import { formatContractName, formatSubgraphName, getSubgraphBasename, } from '../command-helpers/subgraph.js';
        ...
        promptManager.addStep({
            ...
            result: async (address) => {
                ...
                if (protocolInstance.hasABIs() && !initAbi) {
                    const sourcifyContractInfo = await withSpinner('Fetching ABI from Sourcify API...', 'Failed to fetch ABI', 'Warning fetching ABI', () => contractService.getFromSourcify(protocolInstance.getABI(), network.id, address));
    ...
    if (isComposedSubgraph) {
        try {
            ...
            if (immutableEntities.length === 0) {
                this.error('Source subgraph must have at least one immutable entity. This subgraph cannot be used as a source subgraph since it has no immutable entities.', { exit: 1 });
            }
        }
        catch (e) {
