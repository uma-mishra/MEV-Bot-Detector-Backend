...
const validateEntityDirectiveImmutableArgument = (def) => {
    ...
    const hasImmutableArg = entity.arguments?.find(arg => arg.name.value === 'immutable') !== undefined;
    const hasTimeseriesArg = entity.arguments?.find(arg => arg.name.value === 'timeseries') !== undefined;
    ...
    return immutable.fromJS([
        {
            loc: def.loc,
            entity: def.name?.value,
            message: `@entity directive requires \`immutable\` argument\n  ` +
                print.colors.yellow(`Hint: Try updating the entity definition with: @entity(immutable: true)`),
        },
    ]);
};
...
const typeDefinitionValidators = {
    ObjectTypeDefinition: (defs, def) => {
        ...
        return def.name && def.name.value == RESERVED_TYPE
            ...
            : List.of(...validateEntityDirective(def), ...validateEntityDirectiveImmutableArgument(def), ...validateEntityID(def), ...validateEntityFields(defs, def), ...validateNoImportDirective(def), ...validateNoFulltext(def));
    },
